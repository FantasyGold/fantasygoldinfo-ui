<template>
  <div class="columns is-multiline transaction-item">
        ...
        <div v-for="input in inputs" class="is-clearfix">
          ...
          <div class="script">
            ...
            <div v-if="input.scriptSig.asm">
              ...
              <code class="value"><!--
                -->{{ input.scriptSig.asm | fantasygold-script }}<!--
              --></code>
            </div>
      ...
      <template v-if="collapsed">
        <div v-for="(output, index) in outputs" class="is-clearfix">
          ...
          <span class="is-pulled-right amount" v-if="output.value !== '0'">
            ...
            {{ output.value | fantasygold(8) }} FGC
          </span>
          ...
          <div class="script">
            ...
            <div v-if="output.scriptPubKey.asm">
              ...
              <code class="value" :class="{'script-code': contractInfo[index]}"
                ...
                -->{{ output.scriptPubKey.asm | fantasygold-script }}<!--
              --></code>
            </div>
    ...
    <template v-for="({address, name, symbol, decimals, from, to, value}, index) in fgc20TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === fgc20TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
    ...
    <template v-for="({address, name, symbol, from, to, tokenId}, index) in fgc721TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === fgc721TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
          <AddressLink v-if="from" :address="from" class="is-pulled-left" :highlight="highlightAddress" />
          <template v-else>{{ $t('contract.token.mint_tokens') }}</template>
    ...
    <div class="column is-full has-text-right collapse-bottom" v-if="fees !== '0'">
      <template v-if="fees > 0">
        {{ $t('transaction.fee') }} <span class="amount fee">{{ fees | fantasygold }} FGC</span>
      </template>
      <template v-else-if="fees < 0">
        {{ $t('transaction.reward') }} <span class="amount fee">{{ -fees | fantasygold }} FGC</span>
      </template>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      fgc20TokenTransfers() {
        return this.transaction.fgc20TokenTransfers
      },
      fgc721TokenTransfers() {
        return this.transaction.fgc721TokenTransfers
      },
      contractInfo() {
