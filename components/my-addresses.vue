<template>
  <div v-show="addresses.length">
    <button class="button is-fantasygold" @click="show = !show">
    ...
    <table v-show="show">
      <tbody>
        <tr v-for="{address, balance} in list">
          ...
          <td class="monospace has-text-right">{{ balance | fantasygold(8) }}</td>
          <td>
      ...
      <tfoot v-if="addresses.length > 1">
        <tr>
          ...
          <td class="monospace has-text-right">{{ totalBalance | fantasygold(8) }}</td>
          <td></td>
        </tr>
      </tfoot>
    </table>
